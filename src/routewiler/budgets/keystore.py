...
Private keys are stored at ``~/.routewiler/keys/env_<id>.ed25519`` (raw 32-byte
...
_DEFAULT_KEYSTORE_ROOT = Path.home() / ".routewiler" / "keys"
...
class EnvelopeKeystore:
    ...
    Args:
        root: Directory that holds the key files.  Defaults to
              ``~/.routewiler/keys``.  Tests should pass ``tmp_path / "keys"``.
    """
