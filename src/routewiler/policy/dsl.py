...
from routewiler.normalized import Rail, Scheme
...
class DefaultBlock(RoutewilerModel):
    """The fallback rail when no rule matches."""

    rail: Rail


class RuleMatch(RoutewilerModel):
...
class PolicyRule(RoutewilerModel):
...
class PolicyDocument(RoutewilerModel):
...
class PolicyFile:
    ...
    Usage::
        ...
        client = Routewiler(policy=policy, ...)
